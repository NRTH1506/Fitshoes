...
export default function CartPage() {
  ...
  async function handleCheckout() {
    ...
    try {
      const res = await createZaloPayOrder(total, cart, user?._id || user?.id || 'guest');
    ...
    <section style={{ padding: '3rem 5%', minHeight: '60vh' }}>
      ...
      <div className="cart-grid">
        ...
        <div style={{ background: '#fff', padding: '2rem', borderRadius: '.8rem', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', position: 'sticky', top: '2rem' }}>
          ...
          <button onClick={handleCheckout} style={{ width: '100%', background: user ? 'linear-gradient(135deg,#667eea,#764ba2)' : '#ccc', color: '#fff', padding: '1.1rem', border: 'none', borderRadius: '.8rem', fontSize: '1.5rem', fontWeight: 700, cursor: 'pointer', marginBottom: '1rem' }}>
